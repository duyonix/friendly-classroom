...
export default function VerticalListHomework(props) {
  ...
  const renderTopics = (topic, index = true) => {
    return (
      <Box {(index===true)?(key = index}:""} className="section">
        <Typography variant="h6">{topic.topic}</Typography>
        {topic.homeworks.map((homework, index) => {
          return (
            <HomeworkCard key={index + 3000} type={type} homework={homework} />
          );
        })}
      </Box>
    );
