...
function ListMember() {
  ...
  const dataDelete = useSelector((state) => state.peopleReducer?.data);
  const loadingDelete = useSelector((state) => state.peopleReducer?.loading);
  const errDelete = useSelector((state) => state.peopleReducer?.err);
  ...
  const handleDelete = () => {
    dispatch(actDeleteStudent(id, studentDelete.id));
  };
  const handleClose = () => {
    setOpen(false);
  };
