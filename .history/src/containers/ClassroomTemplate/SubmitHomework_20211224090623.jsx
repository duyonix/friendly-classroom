...
import { actSubmitHomework } from "../../redux/modules/SubmitHomework/action";
...
function SubmitHomework(props) {
  ...
  const renderError = () => {
    if (errSubmitHomework) {
      return <Alert severity="error">{errSubmitHomework.response.data.message}</Alert>;
    }
  };
  const renderSuccess = () => {
    if (dataSuberrSubmitHomework) {
      return <Alert severity="success">{dataSuberrSubmitHomework.message}</Alert>;
    }
  };
